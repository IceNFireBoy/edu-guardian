 ...
 */
const StudyRecommendation = ({ subjectsData }) => {
  if (!subjectsData || Object.keys(subjectsData).length === 0) {
    return null; // If no data, don't show any recommendations
  }
  
  // Generate recommendations based on subject data
  const getRecommendations = () => {
    const recommendations = [];
    
    // Check for subjects with many viewed but not completed notes
    ...
    // Check for subjects with many "Need Help" feedbacks
    Object.entries(subjectsData).forEach(([subject, data]) => {
  ...
  // Different colors based on the recommendation type
  ...
  // Take top 3 recommendations at most
